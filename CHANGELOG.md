...
- Subscription segments now refresh after active turns without background polling and calculate reset labels from the latest observation time.

## [0.15.1] - 2026-09-04

### Changed

- Updated `@juanibiapina/pi-extension-settings` to 0.10.0
...
- tokens segment: cache read/write totals and the latest request's cache hit rate.

## [0.13.0] - 2026-07-20

### Added

- xAI/Grok SuperGrok usage in the status bar (via `@juanibiapina/pi-usage` 0.3.0)

### Changed

- Local extension settings support (via `@juanibiapina/pi-extension-settings` 0.9.0)
...
- Removed debug logging from all powerbar producers and core
